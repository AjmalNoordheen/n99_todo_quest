...
const submitLogin = async (req, res) => {
  try {
    ...
    let token = authentication.generateToken(email);
...
const saveChatMessage = async (message, chatId) => {
  try {
    await db
      .promise()
      .query(
        "INSERT INTO discussions (senderEmail, receiverEmail, message) VALUES (?, ?, ?)",
        [message.senderEmail, chatId, message.message]
      );
...
const getChatMessages = async (req, res) => {
  try {
    ...
    const [chatMessages] = await db
      .promise()
      .query(
        "SELECT * FROM discussions WHERE (senderEmail = ? AND receiverEmail = ?) OR (senderEmail = ? AND receiverEmail = ?) ORDER BY createdAt",
